...
public class EchoGraphicsEngine {
	...
	public void load_host_projection() {
		...
		m_curr_matrix = new Matrix3(m_host_matrix).multiply_right(m_view_matrix[m_curr_view]);
	}
	...
	public void change_view() { m_curr_matrix = new Matrix3(m_host_matrix).multiply_right(m_view_matrix[m_curr_view]); }
	public void translate(double x, double y) { m_view_matrix[m_curr_view].translate(x, y); }
	public void rotate(double alpha) { m_view_matrix[m_curr_view].rotate(alpha); }
	public void scale(double sx, double sy) { m_view_matrix[m_curr_view].scale(sx, sy); }
	
	public void push_matrix() {
		if(m_curr_view == MATRIX_STACK_DEPTH - 1) { Program.log.error("EchoGraphicsEngine: matrix stack overflow"); }
		
		m_view_matrix[m_curr_view + 1] = new Matrix3(m_view_matrix[m_curr_view]);
		++m_curr_view;
	}
